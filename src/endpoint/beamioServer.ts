...
const POS_LEDGER_TOPUP_METHOD_BY_CAT_HEX: ReadonlyMap<string, string> = (() => {
	const m = new Map<string, string>()
	const add = (names: string[], label: string) => {
		for (const n of names) m.set(posLedgerKeccakCategory(n), label)
	}
...
function posLedgerTopupPaymentMethodFromCategoryHex(catHex: string): string {
...
function posLedgerPaymentMethodLabel(row: PosLedgerRowForMethod): string {
	...
	try {
		const dj = JSON.parse(row.displayJson || '{}') as { title?: string }
		const t = String(dj.title || '').toLowerCase()
		if (t.includes('usdc merchant')) return 'USDC'
	} catch {
		/* ignore */
	}
	try {
		...
		if (usdc6 > 0n && fiat6 === 0n) return 'USDC'
	} catch {
		/* ignore */
	}
	return 'Card'
...
async function enrichPosLedgerItemsForClients(
	items: Omit<PosLedgerSimplifiedClientItem, 'payerBeamioTag' | 'paymentMethodLabel'>[],
	...
	for (const it of items) {
		...
		try {
			payerSet.add(ethers.getAddress(p).toLowerCase())
		} catch {
			/* ignore */
		}
	}
	const tagBy = new Map<string, string>()
	await Promise.all(
		[...payerSet].map(async (addr) => {
			try {
				const tag = await fetchBeamioTagForEoa(addr)
				if (tag) tagBy.set(addr, tag)
			} catch {
				/* ignore */
			}
		}),
	...
	return items.map((it) => {
		...
		const p = String(it.payer || '').trim()
		if (p && ethers.isAddress(p)) {
			try {
				payerBeamioTag = tagBy.get(ethers.getAddress(p).toLowerCase())
			} catch {
				/* ignore */
			}
		}
...
const BURN_CHARGE_REWARD_BY_ADMIN_SELECTOR = '0x' + ethers.id('burnChargeRewardByAdmin(address,uint256)').slice(2, 10)
...
const tryParseMintPointsByAdminArgs = (data: string): { recipient: string; points6: bigint } | null => {
	try {
		...
		if (decoded?.name === 'mintPointsByAdmin' && decoded.args[0] != null && decoded.args[1] != null) {
			const { totalPoints6 } = unpackTopupMintAmount(BigInt(decoded.args[1]))
			return { recipient: decoded.args[0] as string, points6: totalPoints6 }
		}
	} catch { /* ignore */ }
	return null
}
...
const tryParseBurnChargeRewardByAdminArgs = (data: string): { target: string; points6: bigint } | null => {
	try {
...
> {
	try {
		...
		if (kind === 'merchant') {
			...
			if (!raw.active || raw.claimed || cancelled) return { success: false, error: 'Start Kit redeem code is not active' }
			const member = await registry.members(account)
...
type ChargeSessionState =
	...
	| 'awaiting_topup_auth'
	/** USDC 已 settle；POS 两阶段 topup：顾客再贴卡/扫码后由 `nfcTopup`+`usdcTopupSessionId` 完成 mint */
...
const routing = ( router: Router ) => {
	...
	const upsertNfcHoldingsFromTrustedAssets = async (
		tagIdHex: string,
		uid: string | null,
		ownerEoa: string,
		result: Awaited<ReturnType<typeof fetchUIDAssetsForEOA>>
	): Promise<void> => {
	...
	router.post('/getUIDAssets', async (req, res) => {
		...
		try {
			...
			if (nfcSunTagIdHex) {
				const known = await listNfcBeamioUserCardHoldingsByTagId(nfcSunTagIdHex)
				if (known.length > 0) {
					uidAssetsOpts = {
						...uidAssetsOpts,
						extraCardAddresses: [
	...
	router.get('/nfcBeamioUserCards', async (req, res) => {
		const { uid, e, c, m, tagIdHex } = req.query as {
			uid?: string
			e?: string
			c?: string
			m?: string
		...
		const cards = await listNfcBeamioUserCardHoldingsByTagId(tag)
	...
	router.post('/burnChargeRewardByAdminPrepare', async (req, res) => {
		...
		if (!allow.ok) {
			logger(Colors.yellow(`[burnChargeRewardByAdminPrepare] Cluster REJECT: ${allow.error}`))
			return res.status(400).json({ success: false, error: allow.error }).end()
		}
		const result = await burnChargeRewardByAdminPreparePayload({
			cardAddress: cardAddress ?? '',
			target: target ?? '',
			amount: amount ?? '0',
		})
	...
	router.get('/merchantCardCouponBurnSupported', async (req, res) => {
		...
		try {
			const supportsBurn = await merchantCardSupportsCouponBurn(cardAddress)
	...
	router.post('/cardCouponPosConsumeSubmit', async (req, res) => {
		const pre = await cardCouponPosConsumeSubmitPreCheck(req.body)
		if (!pre.success) {
			logger(Colors.red(`server /api/cardCouponPosConsumeSubmit preCheck FAIL: ${pre.error}`), inspect(req.body, false, 2, true))
			return res.status(400).json({ success: false, error: pre.error }).end()
		}
		const out = pre.preChecked
		logger(
			Colors.green(`server /api/cardCouponPosConsumeSubmit preCheck OK, forwarding to master executeForAdmin`),
	...
	router.post('/nfcTopupPrepare', async (req, res) => {
		const { uid, wallet, beamioTag, amount, currency, cardAddress, e, c, m, membershipTierIndex, membershipFeeFiat6, paidAmount } = req.body as {
			uid?: string
			wallet?: string
			beamioTag?: string
			amount?: string
			...
			/** Principal fiat only (no promotion bonus); used to pack #13 base when card supports it. */
		...
		try {
			...
			if (isBurnIssuedNft) {
				...
				if (!burnBunit.success || !burnBunit.cardOwnerEOA || !burnBunit.feeBUnits6) {
					logger(Colors.red(`[nfcTopup] coupon burn B-Unit pre-check FAIL: ${burnBunit.error}`))
			...
			const usdcTopupSid =
				typeof (req.body as { usdcTopupSessionId?: unknown }).usdcTopupSessionId === 'string'
					? (req.body as { usdcTopupSessionId: string }).usdcTopupSessionId.trim().toLowerCase()
					: ''
			if (usdcTopupSid) {
				if (!isMint) {
				...
				let sess: { ok?: boolean; state?: string; total?: string; cardAddr?: string; USDC_tx?: string }
				try {
					sess = JSON.parse(sg.body) as typeof sess
				} catch {
				...
				if (!sess.cardAddr || ethers.getAddress(sess.cardAddr) !== cardAddress) {
					return res.status(400).json({ success: false, error: 'USDC session card mismatch' }).end()
				}
				const parseSessTot = (raw: string | undefined): bigint => {
					const t = String(raw ?? '')
						.trim()
						.replace(/,/g, '')
					if (!t) return -1n
	...
	router.post('/nfcUsdcTopup', async (req, res) => {
		...
		try {
			...
			if (posAddr && sessionPath) {
				try {
					const cAdm = new ethers.Contract(cardAddr, ['function isAdmin(address) view returns (bool)'], providerBase)
			...
			if (!hasFullNfc && sessionPath) {
				...
				logger(
					Colors.green(
						`[nfcUsdcTopup/phase1] settle OK card=${cardAddr} payer=${settled1.payer} usdc6=${settled1.usdcAmount6} USDC_tx=${settled1.USDC_tx} sid=${sidNorm}`
					)
				)
				sessionUpdate({
			...
			if (!hasFullNfc) {
				logger(Colors.red(`[nfcUsdcTopup] internal: !hasFullNfc without sessionPath`))
				if (!res.headersSent) {
			...
			logger(
				Colors.green(
					`[nfcUsdcTopup] settle OK card=${cardAddr} payer=${settled.payer} usdc6=${settled.usdcAmount6} USDC_tx=${settled.USDC_tx} ` +
			...
			if (sessionPath) {
				...
				if (!settled.USDC_tx || !/^0x[0-9a-fA-F]{64}$/.test(settled.USDC_tx)) {
					logger(Colors.red(`[nfcUsdcTopup/orchestrator] sid=${sidNorm ?? 'n/a'} invalid USDC_tx; abort`))
					sessionUpdate({ state: 'error', error: 'USDC settle returned invalid tx hash' })
					return
				}
				const sidForAwait = sidNorm ?? ''
				void runUsdcNfcTopupPosOrchestrator({
					sid: sidForAwait,
					cardAddr,
					cardOwner: payToOwner,
					currency: cur,
					...
					awaitTopupSignature: async (timeoutMs: number) => {
						if (!sidForAwait) return { ok: false as const, error: 'sid missing' }
						...
						return { ok: false as const, error: `timeout ${timeoutMs}ms waiting POS topup auth` }
					},
				}).catch((err: unknown) => {
					const msg = err instanceof Error ? err.message : String(err)
					logger(Colors.red(`[nfcUsdcTopup/orchestrator] sid=${sidForAwait.slice(0, 8)}… ${msg}`))
...
IMPORTANT: Reply in the SAME language as the user. If user asks in English, use English for text content. If user asks in 中文, use 中文. Match the user's language for all text responses.${feedbackPrompt}`
	...
	/** cardAddAdminBatch：owner 一次签名、一次 executeForOwner，批量登记 Stripe fulfillment admins。 */
	router.post('/cardAddAdminBatch', async (req, res) => {
		const preCheck = await cardAddAdminBatchPreCheck(req.body)
		if (!preCheck.success) {
			logger(Colors.red(`server /api/cardAddAdminBatch preCheck FAIL: ${preCheck.error}`), inspect(req.body, false, 2, true))
			return res.status(400).json({ success: false, error: preCheck.error }).end()
		}
		logger(Colors.green(`server /api/cardAddAdminBatch preCheck OK, forwarding one batch transaction`), inspect({ cardAddress: req.body?.cardAddress }, false, 2, true))
		postLocalhost('/api/executeForOwner', req.body, res)
	})
	...
	router.post('/cardRedeemPreCheck', async (req, res) => {
		...
		const preCheck = await cardRedeemPreCheck({ cardAddress: resolvedCard, redeemCode, toUserEOA })
		if (!preCheck.success) {
			logger(Colors.red(`server /api/cardRedeemPreCheck FAIL: ${preCheck.error}`), { cardAddress: resolvedCard, toUserEOA })
	...
	router.post('/cardRedeem', async (req, res) => {
		...
		if (posOperator != null && posOperator !== '' && !ethers.isAddress(posOperator)) {
			return res.status(400).json({ success: false, error: 'Invalid posOperator address' })
		}
		const resolvedCard = OLD_CCSA_REDIRECTS.includes(cardAddress.toLowerCase()) ? BASE_CCSA_CARD_ADDRESS : cardAddress
		const preCheck = await cardRedeemPreCheck({ cardAddress: resolvedCard, redeemCode, toUserEOA })
		if (!preCheck.success) {
			logger(Colors.red(`server /api/cardRedeem preCheck FAIL: ${preCheck.error}`), { cardAddress: resolvedCard, toUserEOA })
			return res.status(403).json({ success: false, error: preCheck.error }).end()
		}
		logger(Colors.green(`server /api/cardRedeem preCheck OK, forwarding to master`), { cardAddress: resolvedCard, toUserEOA })
		postLocalhost(
			'/api/cardRedeem',
			{
				cardAddress: resolvedCard,
				redeemCode: String(redeemCode).trim(),
				toUserEOA: ethers.getAddress(toUserEOA),
	...
	router.post('/cardCouponOpenClaim', async (req, res) => {
		const preCheck = await cardCouponOpenClaimPreCheck(req.body)
		if (!preCheck.success) {
			logger(Colors.red(`server /api/cardCouponOpenClaim preCheck FAIL: ${preCheck.error}`), inspect(req.body, false, 2, true))
			return res.status(400).json({ success: false, error: preCheck.error }).end()
		}
		logger(
			Colors.green(`server /api/cardCouponOpenClaim preCheck OK, forwarding to master`),
			inspect(
				{
					cardAddress: preCheck.preChecked.cardAddress,
	...
	router.post('/redeemSeries', async (req, res) => {
		...
		const preCheck = await cardRedeemPreCheck({ cardAddress: resolvedCard, redeemCode, toUserEOA })
		if (!preCheck.success) {
			logger(Colors.red(`server /api/redeemSeries preCheck FAIL: ${preCheck.error}`), { cardAddress: resolvedCard, toUserEOA })
			return res.status(403).json({ success: false, error: preCheck.error }).end()
		}
		logger(Colors.green(`server /api/redeemSeries preCheck OK, forwarding to master`), { cardAddress: resolvedCard, toUserEOA })
		postLocalhost(
			'/api/cardRedeem',
			{
				cardAddress: resolvedCard,
	...
	router.post('/updateCardShareMetadata', async (req, res) => {
		...
		const existingRow = await getBeamioCardRowForMetadataSync(cardAddress)
		if (!existingRow) {
			const onChain = await lookupOnChainMerchantCardRegistryIdentity(cardAddress)
	...
	router.post('/AAtoEOA', async (req, res) => {
		...
		if (body.openContainerPayload) {
			...
			try {
				...
				const chainOpenRelayedNonce = await readContainerNonceFromAAStorage(merchantCardProvider, acc, 'openRelayed')
	...
	router.post('/cancelRequest', async (req, res) => {
		...
		router.get('/checkBUnitClaimEligibility', async (req, res) => {
			...
			try {
				const { canClaim, nonce, reason } = await resolveBUnitFreeClaimEligibility(providerConet, address)
	...
	router.post('/claimBUnits', async (req, res) => {
		...
		const preCheck = await claimBUnitsClusterPreCheck(providerConet, body)
		if (!preCheck.success) {
			logger(Colors.red(`server /api/claimBUnits cluster preCheck FAIL (${preCheck.code}): ${preCheck.error}`))
			return res.status(400).json({ success: false, error: preCheck.error, code: preCheck.code }).end()
		}
		logger(Colors.green('server /api/claimBUnits cluster preCheck OK, forwarding to master'))
	...
	router.post('/aaCreateViaEntryPointSubmit', async (req, res) => {
		...
		if (!body.eoa || !ethers.isAddress(body.eoa)) {
			return res.status(400).json({ success: false, error: 'Invalid eoa: require valid 0x address' })
		}
		...
		try {
			...
			logger(Colors.red('[aaCreateViaEntryPointSubmit] forward error:'), e?.message ?? e)
	...
	router.get('/getBUnitLedger', async (req, res) => {
		...
		try {
			...
			const indexer = new ethers.Contract(BEAMIO_INDEXER_DIAMOND, INDEXER_ABI, providerConet)
  ...
  if (window.opener) {
	...
	router.get('/og/s/:token', async (req, res) => {
		...
		try {
			...
			logger(Colors.red('[og/s/:token] error:'), err?.message ?? err)
	...
	router.get('/og/coupon-claim.jpg', async (req, res) => {
		...
		try {
			...
			logger(Colors.red('[og/coupon-claim.jpg] error:'), err?.message ?? err)
			return res.status(500).json({ error: 'Failed to render coupon share image' })
		}
	})
