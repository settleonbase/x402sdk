...
import { enqueueChatIndexPointerRelay } from '../chatIndexPointer'
import {
...
import {
	validatorDepositRedeemCancelPool,
...
function seriesMetadataMatchesIssuedCoupon(
	metadata: unknown,
	couponId: string,
	issuedTokenIdNorm: string
): boolean {
	...
	if (rootId === couponId) {
		if (rootTok === '' || rootTok === issuedTokenIdNorm) return true
	}
	...
	if (props && typeof props === 'object' && !Array.isArray(props)) {
		...
		if (bc && typeof bc === 'object' && !Array.isArray(bc)) {
			...
			if (bid === couponId) {
				if (btok === '' || btok === issuedTokenIdNorm) return true
			}
		}
	}
	return false
...
const routing = ( router: Router ) => {
	...
	router.get('/debug/ip', (req, res) => {
				...
				const status =
					err instanceof CreateInstitutionalAaHttpError
						...
						: typeof err?.statusCode === 'number'
							? err.statusCode
							: 500
				logger(Colors.red('[createInstitutionalAa] error:'), err?.message ?? err)
		...
		router.post('/aaCreateViaEntryPointSubmit', async (req, res) => {
			...
			try {
				const out = await submitAAAccountCreationViaEntryPoint({
				...
				logger(Colors.red('[aaCreateViaEntryPointSubmit] error:'), err?.message ?? err)
		...
		router.post('/excludeUserCard', async (req, res) => {
			const body = req.body as { cardAddress?: string; excludedBy?: string }
			const cardAddress = body.cardAddress?.trim()
			...
			if (!excludedBy || !ethers.isAddress(excludedBy)) {
				res.status(400).json({ success: false, error: 'Invalid or missing excludedBy' }).end()
				return
			}
			try {
				const r = await applyExcludeUserCard({ cardAddress, excludedBy })
				if (!r.success) {
				...
				logger(Colors.red('[excludeUserCard] error:'), msg)
		...
		router.post('/updateCardMerchantImage', async (req, res) => {
			const body = req.body as { cardAddress?: string; merchantImage?: string }
			const cardAddress = body.cardAddress?.trim()
			...
			try {
				const r = await applyBeamioCardMerchantImageUrlUpdate({
					cardAddress,
					merchantImage,
				})
				if (!r.success) {
				...
				logger(Colors.red('[updateCardMerchantImage] error:'), err?.message ?? err)
		...
		router.post('/updateCardProgramImage', async (req, res) => {
			const body = req.body as { cardAddress?: string; image?: string }
			const cardAddress = body.cardAddress?.trim()
			...
			try {
				const r = await applyBeamioCardProgramImageUrlUpdate({
					cardAddress,
					image,
				})
				if (!r.success) {
				...
				logger(Colors.red('[updateCardProgramImage] error:'), err?.message ?? err)
		...
		router.post('/updateIssuedCouponMetadata', async (req, res) => {
			try {
				...
				if (couponIdx < 0) {
					const seriesProbe = await getSeriesByCardAndTokenId(cardNorm, tokenIdNorm)
					...
					if (!seriesProbe || !seriesMatch) {
						logger(
							Colors.yellow(
								`[updateIssuedCouponMetadata] no shareTokenMetadata coupon match wanted couponId=${couponId} issuedTokenId=${tokenIdNorm} couponsLen=${coupons.length} seriesRow=${Boolean(seriesProbe)} seriesMetaMatch=${seriesMatch} sample=${inspect(keysForLog, false, 4, true)}`
							)
						)
					...
					logger(
						Colors.cyan(
							`[updateIssuedCouponMetadata] series-only path (shareTokenMetadata.coupons empty/mismatch; beamio_nft_series metadata matches) card=${cardNorm} tokenId=${tokenIdNorm}`
						)
					)
				} else {
				...
				if (seriesOnlyPath && !series) {
					logger(
						Colors.red(
							`[updateIssuedCouponMetadata] series row disappeared after probe card=${cardNorm} tokenId=${tokenIdNorm}`
						)
					)
					return res
						.status(500)
				...
				logger(
					Colors.green(
						`[updateIssuedCouponMetadata] done 200 card=${cardNorm} couponId=${couponId} tokenId=${tokenIdNorm} seriesOnlyPath=${seriesOnlyPath} icon=${summarizeCouponIconForDebugLog(String(body.icon ?? ''))}`
					)
				)
		...
		router.post('/aaMultisigOfflineSubmit', (req, res) => {
			const body = req.body as import('../aaMultisigOfflineSubmit').AaMultisigOfflineSubmitForwardBody
			...
			aaMultisigOfflineSubmitPool.push({
				inner: body.inner,
				submitterEoa: ethers.getAddress(body.submitterEoa),
				feePayer: ethers.getAddress(body.feePayer),
				feeAmount,
				dedupeKey: body.dedupeKey,
				res,
			})
			logger(Colors.cyan(`[aaMultisigOfflineSubmit] pushed to pool task=${body.inner.taskId}`))
			kickAaMultisigOfflineSubmitProcess()
		})

		/** Institutional AA V2 — EIP-712 propose / vote (Cluster prechecked). */
		router.post('/aaInstitutionalV2ProposeTransfer', (req, res) => {
			aaInstitutionalV2RelayPool.push({ kind: 'proposeTransfer', body: req.body as Record<string, unknown>, res })
			kickAaInstitutionalV2RelayProcess()
		})
		router.post('/aaInstitutionalV2ProposeSetPolicy', (req, res) => {
			aaInstitutionalV2RelayPool.push({ kind: 'proposeSetPolicy', body: req.body as Record<string, unknown>, res })
			kickAaInstitutionalV2RelayProcess()
		})
		router.post('/aaInstitutionalV2Vote', (req, res) => {
			aaInstitutionalV2RelayPool.push({ kind: 'vote', body: req.body as Record<string, unknown>, res })
			kickAaInstitutionalV2RelayProcess()
		})
		...
		router.post('/cardOpenTransfer', (req, res) => {
			...
			cardOpenTransferPool.push({
				cardAddress: body.cardAddress as string,
				fromEOA: body.fromEOA as string,
				to: body.to as string,
				id: body.id as string,
				amount: body.amount as string,
				maxAmount: body.maxAmount as string,
				validAfter: body.validAfter as string,
				validBefore: body.validBefore as string,
				nonce: body.nonce as string,
				signature: body.signature as string,
				res,
			})
			kickCardOpenTransferPoolPress()
		})
		...
		router.post('/cardCouponPosClaimWallet', (req, res) => {
			...
			cardCouponPosClaimWalletPool.push({
				cardAddress,
				couponId,
				userEOA,
				tokenId,
			...
			logger(
				Colors.cyan(
					`[cardCouponPosClaimWallet] pushed to pool, card=${cardAddress} couponId=${couponId} tokenId=${tokenId} posAdmin=${posAdminEOA}`
				)
			)
			cardCouponPosClaimWalletProcess().catch((err: any) => {
				logger(Colors.red('[cardCouponPosClaimWalletProcess] unhandled error:'), err?.message ?? err)
			})
		})
		...
		router.post('/AAtoEOA', (req, res) => {
			...
			logger(`[AAtoEOA] [DEBUG] Master received openContainer=${!!body?.openContainerPayload} requestHash=${body?.requestHash ?? 'n/a'} forText=${body?.forText ? `"${String(body.forText).slice(0, 40)}…"` : 'n/a'} OpenContainerRelayPool.len=${OpenContainerRelayPool.length} ${settlePoolIdleSummary()}`)
		...
		router.post('/relocateBUnitToSmartWallet', (req, res) => {
			const body = req.body as { eoaOwner?: string; aaAccount?: string }
			if (!body.eoaOwner || !body.aaAccount) {
				return res.status(400).json({ success: false, error: 'Missing eoaOwner or aaAccount' }).end()
			}
			relocateBUnitsToSmartWalletPool.push({
				eoaOwner: body.eoaOwner,
				aaAccount: body.aaAccount,
				res,
			})
			relocateBUnitsToSmartWalletProcess().catch((err: any) => {
				logger(Colors.red('[relocateBUnitsToSmartWalletProcess] unhandled error:'), err?.message ?? err)
			})
		})
		...
		router.post('/validatorDepositRedeemAdminCreate', (req, res) => {
			...
			validatorDepositRedeemCreatePool.push({
				contract: ethers.getAddress(b.contract),
				admin: ethers.getAddress(b.admin),
				codeHash: b.codeHash,
		...
		router.post('/validatorDepositRedeemAdminCancel', (req, res) => {
			const b = req.body as { contract?: string; admin?: string; codeHash?: string; nonce?: string; deadline?: string; signature?: string }
			if (!b.contract || !b.admin || !b.codeHash || b.nonce == null || b.deadline == null || !b.signature) {
				return res.status(400).json({ success: false, error: 'Missing validator redeem cancel fields' }).end()
			}
			validatorDepositRedeemCancelPool.push({
				contract: ethers.getAddress(b.contract),
				admin: ethers.getAddress(b.admin),
				codeHash: b.codeHash,
				nonce: BigInt(b.nonce),
			...
			validatorDepositRedeemCancelProcess().catch((err: any) => {
				logger(Colors.red('[validatorDepositRedeemCancelProcess] unhandled error:'), err?.message ?? err)
			})
		})
		...
		router.post('/payByNfcUidPrepare', async (req, res) => {
			...
			logger(
				Colors.cyan(
					`[payByNfcUidPrepare] Master received uid=${uid.trim().slice(0, 16)}... fiat6=${fiat6Ok ? `${fiat6Trim} ${currencyTrim}` : 'none'} usdc6=${usdc6Ok ? amountUsdc6 : 'none'} merchantInfraCard=${merchantInfraForPrepare ?? 'none'}`
				)
			)
		...
		router.post('/payByNfcUidSignContainer', async (req, res) => {
			...
			const merchantInfraForContainer =
				typeof merchantInfraCard === 'string' && merchantInfraCard.trim() && ethers.isAddress(merchantInfraCard.trim())
					? ethers.getAddress(merchantInfraCard.trim())
					: undefined
			logger(Colors.cyan(`[payByNfcUidSignContainer] Master received container uid=${uid.slice(0, 16)}... amountFiat6=${fiat6Ok ? `${fiat6Trim} ${currencyTrim}` : 'none'} amountUsdc6=${usdc6Ok ? amountUsdc6 : 'none'} merchantInfraCard=${merchantInfraForContainer ?? 'none'}\n` + inspect(containerPayload, false, 4, true)))
