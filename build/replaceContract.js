 ...
 */
async function directCallSmoke(provider, facetAddr, selector, label) {
    try {
...
function assertAbiHas(abiJson, signature) {
...
function ensureSelectors(selectors, mustHave) {
...
const ACTION_MUST_HAVE = [
    selectorOf("syncTokenAction((uint8,address,address,address,uint256,uint256,string,string,uint256,uint256,uint256,uint256,uint256,string,string,string))"),
...
let desired = getSelectors(ActionABI_json_1.default);
desired = ensureSelectors(desired, ACTION_MUST_HAVE);
async function printCodeHash(provider, addr, label) {
    const code = await provider.getCode(addr);
    const len = (code.length - 2) / 2;
    const h = ethers_1.ethers.keccak256(code);
...
async function checkAndVerify() {
    ...
    assertAbiHas(ActionABI_json_1.default, "syncTokenAction((uint8,address,address,address,uint256,uint256,string,string,uint256,uint256,uint256,uint256,uint256,string,string,string))");
 ...
 */
async function upgradeActionFacet(NEW_ACTION_FACET) {
    const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
    const wallet = new ethers_1.ethers.Wallet(util_1.masterSetup.settle_contractAdmin[0], provider);
    const installed = await getInstalledSelectors(wallet);
    const desired = getSelectors(ActionABI_json_1.default);
...
const selFromSig = selectorOf("syncTokenAction((uint8,address,address,address,uint256,uint256,string,string,uint256,uint256,uint256,uint256,uint256,string,string,string))");
const selsFromAbi = getSelectors(ActionABI_json_1.default);
