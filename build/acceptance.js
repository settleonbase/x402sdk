...
const PRIVATE_KEY = util_1.masterSetup.settle_contractAdmin[0];
if (!RPC_URL) {
...
// ====== helpers ======
function mustEqual(label, got, want) {
    if (got.toLowerCase() !== want.toLowerCase()) {
        throw new Error(`❌ ${label} mismatch: got=${got} want=${want}`);
    }
    console.log(`✅ ${label} facet ok: ${got}`);
}
async function facetAddress(loupe, selector) {
    return (await loupe.facetAddress(selector));
}
async function main() {
    const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
    const signer = PRIVATE_KEY && PRIVATE_KEY.length > 0
        ? new ethers_1.ethers.Wallet(PRIVATE_KEY, provider)
        : null;
    if (!signer) {
    ...
    const loupe = new ethers_1.ethers.Contract(DIAMOND, DiamondLoupeABI, provider);
    const ownership = new ethers_1.ethers.Contract(DIAMOND, OwnershipABI, provider);
    const catalog = new ethers_1.ethers.Contract(DIAMOND, CatalogABI, provider);
    const action = new ethers_1.ethers.Contract(DIAMOND, ActionABI, provider);
    const stats = new ethers_1.ethers.Contract(DIAMOND, StatsABI, provider);
    const task = new ethers_1.ethers.Contract(DIAMOND, TaskABI, provider);
    const admin = new ethers_1.ethers.Contract(DIAMOND, AdminABI, provider);
    // ===== Owner check =====
    console.log("\n== Owner check ==");
    ...
    console.log("DIAMOND: codeBytes=%d keccak256=%s", (code.length - 2) / 2, ethers_1.ethers.keccak256(code));
    ...
    for (const addr of expected) {
        if (!facetSet.has(addr))
            throw new Error(`❌ missing facet in loupe.facets(): ${addr}`);
    }
    ...
    const agg = await stats.getAggregatedStats(0, ethers_1.ethers.ZeroAddress, 0, 0);
    // ethers v6 兼容：
    // 1) 有些情况返回 { stats: Result(...) }
    // 2) 有些情况直接返回 Result(...)（没有 stats 字段）
    // 3) Result(...) 内部可能有命名字段，也可能只有 [0..3]
    const statsRes = agg.stats ?? agg;
    const toStr = (x) => (x == null ? "null" : x.toString());
    ...
    if (WRITE_TEST) {
        if (!signer)
            throw new Error("WRITE_TEST=1 but PRIVATE_KEY not set");
        ...
        const diamondCut = new ethers_1.ethers.Contract(DIAMOND, DiamondCutABI, signer);
