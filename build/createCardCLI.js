...
const CURRENCIES = ['CAD', 'USD', 'JPY', 'CNY', 'USDC', 'HKD', 'EUR', 'SGD', 'TWD'];
async function main() {
    if (!MemberCard_1.Settle_ContractPool?.length) {
    ...
    if (!cardOwnerRaw || !ethers_1.ethers.isAddress(cardOwnerRaw)) {
        console.error('请设置 CARD_OWNER（有效地址）');
        process.exit(1);
    }
    const cardOwner = ethers_1.ethers.getAddress(cardOwnerRaw);
    ...
    const SC = MemberCard_1.Settle_ContractPool[0];
    ...
    const cardAddress = await (0, CCSA_1.createBeamioCardWithFactory)(factory, cardOwner, currency, priceInCurrencyE6, {});
