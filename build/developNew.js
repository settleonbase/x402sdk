 ...
 * 				diamondAddress			0x0DBDF27E71f9c89353bC5e4dC27c9C5dAe0cc612
...
const DiamondCutFacetABI_json_2 = __importDefault(require("./ABI/DiamondCutFacetABI.json"));
...
const OwnershipABI_json_2 = __importDefault(require("./ABI/OwnershipABI.json"));
const developshowSelectorsPlusCuts_1 = require("./developshowSelectorsPlusCuts");
const logger_1 = require("./logger");
const util_2 = require("./util");
const LoupeABI = DiamondLoupeFacet_json_1.default;
const RPC_URL = (0, util_2.resolveBeamioConetHttpRpcUrl)();
...
const MAX_HOURS_SELECTOR = ethers_1.ethers.id("MAX_HOURS()").slice(0, 10).toLowerCase();
// ---------- helpers ----------
...
function getSelectors(abiJson) {
    const abi = abiArray(abiJson);
    const selectors = abi
        .filter((f) => f.type === "function")
        .map((f) => ethers_1.ethers.id(`${f.name}(${(f.inputs || []).map((i) => i.type).join(",")})`).slice(0, 10));
    return [...new Set(selectors)].map((s) => s.toLowerCase());
}
function buildCut(facetAddress, action, selectors) {
...
async function hasLoupe(wallet) {
    const loupe = new ethers_1.ethers.Contract(DIAMOND, abiArray(LoupeABI), wallet);
    try {
        await loupe.facets();
        return true;
    }
        ...
        if (msg.includes("fn not found") || msg.includes("Diamond: fn not found"))
            return false;
        throw e;
    }
}
async function diamondCutTx(wallet, cuts, tag) {
    if (cuts.length === 0) {
        console.log(`== ${tag}: nothing to cut ==`);
        return;
    }
    const diamondCut = new ethers_1.ethers.Contract(DIAMOND, abiArray(DiamondCutFacetABI_json_1.default), wallet);
...
async function getInstalledSelectors(wallet) {
 ...
 */
function planCutsForFacet(desiredSelectors, installed, targetFacet) {
    const add = [];
    const replace = [];
    const skip = [];
    const forbidden = [];
...
async function mustEqual(label, got, want) {
...
function mustInstalled(label, got) {
...
async function check(wallet) {
    const ownership = new ethers_1.ethers.Contract(DIAMOND, abiArray(OwnershipABI_json_1.default), wallet);
...
const planList = [
    { name: "AdminFacet", facet: FACETS.AdminFacet, abi: adminFacet_ABI_json_1.default },
    { name: "TaskFacet", facet: FACETS.task, abi: TaskABI_json_1.default },
    { name: "StatsFacet", facet: FACETS.stats, abi: StatsABI_json_1.default },
    { name: "CatalogFacet", facet: FACETS.catalog, abi: CatalogABI_json_1.default },
    { name: "ActionFacet", facet: FACETS.action, abi: ActionABI_json_1.default },
    { name: "DiamondCutFacet", facet: FACETS.DiamondCut, abi: DiamondCutFacetABI_json_2.default },
    { name: "DiamondLoupeFacet", facet: FACETS.loupe, abi: DiamondLoupeFacet_json_1.default },
    { name: "OwnershipFacet", facet: FACETS.ownership, abi: OwnershipABI_json_2.default },
];
// ---------- main upgrade flow ----------
async function main() {
    const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
    const wallet = new ethers_1.ethers.Wallet(util_1.masterSetup.settle_contractAdmin[0], provider);
    ...
    if (!loupeInstalled) {
        ...
        const cuts1 = [
            ...
            buildCut(FACETS.ownership, 0, getSelectors(OwnershipABI_json_1.default)),
        ];
    ...
    for (const x of planList) {
        ...
        console.log(`${x.name}: desired=${desired.length} add=${add.length} replace=${replace.length} skip=${skip.length} forbidden=${forbidden.length}`);
        // 1) Replace first (if any)
        if (replace.length) {
            await diamondCutTx(wallet, [buildCut(x.facet, 1, replace)], `${x.name}-Replace`);
            for (const sel of replace)
                installed.set(sel, x.facet.toLowerCase());
        }
        // 2) Add new selectors
        if (add.length) {
...
async function testStatsRecordDetailedActivityAt() {
    const stats = new ethers_1.ethers.Contract(DIAMOND, abiArray(StatsABI_json_1.default), wallet);
    // 写入需要 owner，所以 wallet 必须是 owner 才会成功
    const now = Math.floor(Date.now() / 1000);
    const tx = await stats.recordDetailedActivityAt(now, "0x0000000000000000000000000000000000000001", // card
...
async function verifyFacetAddressOfSelector(selector) {
    const loupe = new ethers_1.ethers.Contract(DIAMOND, abiArray(LoupeABI), wallet);
...
async function replaceStatsAll() {
    ...
    if (!NEW_STATS_FACET)
        throw new Error("EXPECT.stats not set");
    const statsSelectors = getSelectors(StatsABI_json_1.default);
...
async function replaceCatalogAll() {
    ...
    if (!NEW_CATALOG_FACET)
        throw new Error("EXPECT.catalog not set");
    const installed = await getInstalledSelectors(wallet);
    const desired = getSelectors(CatalogABI_json_1.default);
    // forbidden: diamondCut（通常不在这个 ABI 里，但保险起见）
    // 如果你担心 MAX_HOURS() 冲突，可以把 MAX_HOURS_SELECTOR 加进 forbidden
    const forbidden = new Set([DIAMOND_CUT_SELECTOR /*, MAX_HOURS_SELECTOR*/]);
    const toReplace = [];
    const toAdd = [];
    ...
    if (skippedForbidden.length)
        console.log("forbidden selectors =", skippedForbidden);
    if (toReplace.length) {
        await diamondCutTx(wallet, [{ facetAddress: NEW_CATALOG_FACET, action: 1, functionSelectors: toReplace }], "CatalogFacet-Replace");
    }
    if (toAdd.length) {
        await diamondCutTx(wallet, [{ facetAddress: NEW_CATALOG_FACET, action: 0, functionSelectors: toAdd }], "CatalogFacet-Add");
    }
...
async function checkAndVerify() {
    ...
    const loupe = new ethers_1.ethers.Contract(DIAMOND, abiArray(LoupeABI), wallet);
    const ownership = new ethers_1.ethers.Contract(DIAMOND, abiArray(OwnershipABI_json_1.default), wallet);
    const catalog = new ethers_1.ethers.Contract(DIAMOND, abiArray(CatalogABI_json_1.default), wallet);
    const action = new ethers_1.ethers.Contract(DIAMOND, abiArray(ActionABI_json_1.default), wallet);
    console.log("== Owner check ==");
    console.log("diamond owner =", await ownership.owner());
    console.log("wallet       =", await wallet.getAddress());
    console.log("\n== Facet route checks (Loupe.facetAddress) ==");
    // ---- Catalog selectors ----
    const sel_catalog_register = selectorOf("registerCard((address,address,string,string,string,uint8,uint256,uint8,uint64,uint64,uint256))");
    const sel_catalog_update = selectorOf("updateCardMeta((address,string,string,string,uint8,uint256,uint8,uint64,uint64,uint256))");
    ...
    // ---- Action selectors ----
    const sel_action_sync = selectorOf("syncTokenAction((uint8,address,address,address,uint256,uint256,string,string,uint256,uint256,uint256,uint256,uint256,string,string,string))");
...
function fnSelector(iface, nameOrSig) {
    const f = iface.getFunction(nameOrSig);
    if (!f)
        throw new Error(`Function not found in ABI: ${nameOrSig}`);
...
async function fixCatalogWriteSelectors() {
    ...
    const catalogIface = new ethers_1.ethers.Interface(abiArray(CatalogABI_json_1.default));
    const sels = [
    ...
    if (toReplace.length) {
        await diamondCutTx(wallet, [buildCut(NEW_CATALOG_FACET, 1, toReplace)], "CatalogWrite-Replace");
    }
    if (toAdd.length) {
        await diamondCutTx(wallet, [buildCut(NEW_CATALOG_FACET, 0, toAdd)], "CatalogWrite-Add");
    }
...
async function fixActionSyncTokenActionSelector() {
    ...
    const actionIface = new ethers_1.ethers.Interface(abiArray(ActionABI_json_1.default));
    ...
    const action = current === ethers_1.ethers.ZeroAddress ? 0 : 1;
    console.log(`to${action === 0 ? "Add" : "Replace"} = 1`, "current =", current);
    // 4) diamondCut
    await diamondCutTx(wallet, [buildCut(FACETS.action, action, [sel])], `ActionSync-${action === 0 ? "Add" : "Replace"}`);
...
async function debugActionReadRouting() {
    ...
    const wallet = new ethers_1.ethers.Wallet(util_1.masterSetup.settle_contractAdmin[0], provider);
    const loupe = new ethers_1.ethers.Contract(DIAMOND, abiArray(LoupeABI), wallet);
    const sel = ethers_1.ethers.id("getActionCount()").slice(0, 10).toLowerCase(); // 0x5eecd218
...
async function debugFacetHasCode() {
    ...
    if (code === "0x")
        throw new Error("❌ facet address has NO code (EOA or not deployed)");
}
//   debugFacetHasCode()
async function directFacetSmoke() {
    ...
    const facet = new ethers_1.ethers.Contract(facetAddr, abiArray(ActionABI_json_1.default), wallet);
    // 1) 直接 call facet（不是 diamond）
    const v = await facet.getActionCount();
    console.log("direct facet getActionCount =", v.toString());
    function safeSelector(abiJson, name) {
        const iface = new ethers_1.ethers.Interface(abiArray(abiJson));
        const fn = iface.getFunction(name);
        if (!fn)
            throw new Error(`Function not found in ABI: ${name}`);
        return fn.selector.toLowerCase();
    }
    console.log("ABI selector(getActionCount) =", safeSelector(ActionABI_json_1.default, "getActionCount"));
}
const printAllSelectors = async () => {
    (0, developshowSelectorsPlusCuts_1.showSelectorsPlusCuts)(planList);
};
//printAllSelectors();
//   directFacetSmoke()
const cuts = [
...
const addCuts = async () => {
    const diamondCut = new ethers_1.ethers.Contract(DIAMOND, DiamondCutFacetABI_json_2.default, wallet);
    ...
    try {
        ...
        (0, logger_1.logger)(`addCuts Error!`, ex.message);
    }
};
addCuts();
