...
exports.DEPRECATED_LEGACY_GLOBAL_USER_CARD_LOWER = exports.DEPRECATED_LEGACY_CCSA_CARD_LOWER = exports.API_EXCLUDED_USER_CARD_ADDRESSES = void 0;
exports.registerDynamicApiExcludedUserCard = registerDynamicApiExcludedUserCard;
exports.setDynamicApiExcludedUserCards = setDynamicApiExcludedUserCards;
exports.normalizeUserCardAddressLower = normalizeUserCardAddressLower;
exports.isApiExcludedUserCard = isApiExcludedUserCard;
exports.filterApiExcludedUserCardAddresses = filterApiExcludedUserCardAddresses;
exports.filterApiExcludedCardRows = filterApiExcludedCardRows;
exports.listApiExcludedUserCardAddressesChecksum = listApiExcludedUserCardAddressesChecksum;
 ...
 */
exports.API_EXCLUDED_USER_CARD_ADDRESSES = new Set([
    ...
    /** Merchant program card — operator blacklist */
    '0x4cea622155c82133839a46fe108fcefc5b9d079f',
    ...
    '0x086bdcc6840f2c65f4e6f100c507266339990112',
    /** Merchant program card — operator blacklist (hide from Discover & assets) */
    '0x251fd186e01dfd60828a771196003cf3361517fe',
    /** Merchant program card — operator blacklist (hide from Discover & assets) */
    '0x971f740d78b2602a5ae163c535e52ced54ed7e71',
    /** Merchant program card — operator blacklist (hide from Discover & assets) */
    '0xddd4bb967578aeab78474c4469ee7a5ca0a8b3e8',
    /** Merchant program card — operator blacklist (hide from Discover & assets) */
...
const DYNAMIC_API_EXCLUDED_USER_CARD_ADDRESSES = new Set();
function registerDynamicApiExcludedUserCard(raw) {
...
function setDynamicApiExcludedUserCards(lowers) {
...
exports.DEPRECATED_LEGACY_CCSA_CARD_LOWER = '0x2032a363bb2cf331142391fc0dad21d6504922c7';
/** @deprecated 旧全局卡常量名；新代码请用 isApiExcludedUserCard，勿再引入 infrastructure card 语义。 */
exports.DEPRECATED_LEGACY_GLOBAL_USER_CARD_LOWER = '0xbccfa50d2a5917c7a8662177f5f4b7a175787270';
function normalizeUserCardAddressLower(raw) {
...
function isApiExcludedUserCard(raw) {
    const lower = normalizeUserCardAddressLower(raw);
    if (lower == null)
        return false;
    return exports.API_EXCLUDED_USER_CARD_ADDRESSES.has(lower) || DYNAMIC_API_EXCLUDED_USER_CARD_ADDRESSES.has(lower);
}
function filterApiExcludedUserCardAddresses(addresses) {
    const out = [];
    const seen = new Set();
    for (const raw of addresses) {
        const lower = normalizeUserCardAddressLower(raw);
        if (!lower || isApiExcludedUserCard(lower) || seen.has(lower))
            continue;
        seen.add(lower);
        out.push(ethers_1.ethers.getAddress(raw.trim()));
    }
    return out;
}
function filterApiExcludedCardRows(rows) {
    return rows.filter((row) => !isApiExcludedUserCard(row.cardAddress));
}
/** Cluster `GET /api/excludedUserCards` — UI 动态拉取，勿在客户端硬编码维护。 */
function listApiExcludedUserCardAddressesChecksum() {
    const merged = new Set([...exports.API_EXCLUDED_USER_CARD_ADDRESSES, ...DYNAMIC_API_EXCLUDED_USER_CARD_ADDRESSES]);
    return [...merged].map((lower) => ethers_1.ethers.getAddress(lower));
}
