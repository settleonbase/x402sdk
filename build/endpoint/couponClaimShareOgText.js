...
exports.renderOgTextLayerPng = renderOgTextLayerPng;
exports.buildOgTextComposites = buildOgTextComposites;
...
function ensureOgMultilingualFonts() {
    if (ogFontsReady)
        return;
    for (const spec of OG_FONT_FACES) {
        try {
            const path = nodeRequire.resolve(`${spec.pkg}/files/${spec.file}`);
            canvas_1.GlobalFonts.registerFromPath(path, spec.family);
        }
        catch {
...
function canvasWeight(fontWeight) {
    if (fontWeight >= 700)
        return 700;
    if (fontWeight >= 600)
        return 600;
...
function ogCanvasFont(fontWeight, fontSize) {
...
function renderOgTextLayerPng(layer) {
    ...
    const measureCanvas = (0, canvas_1.createCanvas)(maxWidth, canvasHeight);
    ...
    const overlayWidth = layer.align === 'left' ? Math.min(maxWidth, Math.max(1, measuredWidth)) : maxWidth;
    const canvas = (0, canvas_1.createCanvas)(overlayWidth, canvasHeight);
...
function buildOgTextComposites(layers) {
    return layers
