...
exports.mergeOwnershipSnapshots = mergeOwnershipSnapshots;
exports.readOwnershipSnapshot = readOwnershipSnapshot;
exports.discoverOrphanMembershipNfts = discoverOrphanMembershipNfts;
exports.resolveMergedCardOwnership = resolveMergedCardOwnership;
exports.holderHasValidMembershipNft = holderHasValidMembershipNft;
/**
...
function mergeOwnershipSnapshots(...parts) {
    let points = 0n;
    const byId = new Map();
...
function normalizeOwnershipNfts(nftsRaw) {
    if (!Array.isArray(nftsRaw))
        return [];
    return nftsRaw.map((n) => {
...
async function readOwnershipSnapshot(card, holder) {
    try {
...
async function readExpiry(card, tokenId) {
    try {
...
async function readTierIndexOrMax(card, tokenId) {
    try {
...
async function readMembershipNftRowIfHeld(card, holder, tokenId, nowSec) {
    if (!(0, membershipFeeMetadata_1.isMembershipNftTokenId)(tokenId))
        return null;
    try {
...
async function listMembershipNftTierTokenIdsByCardSafe(cardAddress) {
    try {
        const rows = await (0, db_1.listMembershipNftTierTokenIdsByCard)(cardAddress);
        ...
        return rows
            ...
            .filter((tid) => (0, membershipFeeMetadata_1.isMembershipNftTokenId)(tid));
    }
    catch {
        return [];
    }
}
async function collectMembershipTokenIdCandidates(card, cardAddress, holders) {
    const ids = new Set();
    const add = (raw) => {
        try {
            const tid = typeof raw === 'bigint' ? raw : BigInt(String(raw ?? '').trim() || '0');
            if ((0, membershipFeeMetadata_1.isMembershipNftTokenId)(tid))
    ...
    ids.add(membershipFeeMetadata_1.MEMBERSHIP_NFT_MIN_ID.toString());
    ...
    try {
        ...
        const span = total > 0n && total < MEMBERSHIP_BALANCE_PROBE_HARD_CAP
            ? total
            : MEMBERSHIP_BALANCE_PROBE_SPAN;
        const end = membershipFeeMetadata_1.MEMBERSHIP_NFT_MIN_ID + span;
        for (let tid = membershipFeeMetadata_1.MEMBERSHIP_NFT_MIN_ID; tid < end && tid < membershipFeeMetadata_1.MEMBERSHIP_NFT_MAX_EXCLUSIVE; tid++) {
            ids.add(tid.toString());
        }
    }
    catch {
        for (let i = 0n; i < MEMBERSHIP_BALANCE_PROBE_SPAN; i++) {
            ids.add((membershipFeeMetadata_1.MEMBERSHIP_NFT_MIN_ID + i).toString());
        }
    }
    try {
        const fromDb = await listMembershipNftTierTokenIdsByCardSafe(cardAddress);
        for (const tid of fromDb)
            add(tid);
    }
    catch {
        /* ignore */
 ...
 */
async function discoverOrphanMembershipNfts(input) {
    const holders = Array.from(new Set(input.holders
        .map((h) => {
        try {
            return ethers_1.ethers.getAddress(String(h || '').trim());
        }
        catch {
            return '';
        }
    ...
    const hasMembershipInInventory = existing.some((n) => (0, membershipFeeMetadata_1.isMembershipNftTokenId)(n.tokenId) && !n.isExpired);
    if (hasMembershipInInventory)
        return [];
    const card = new ethers_1.ethers.Contract(input.cardAddress, OWNERSHIP_ABI, input.provider);
    const nowSec = BigInt(Math.floor(Date.now() / 1000));
    const candidates = await collectMembershipTokenIdCandidates(card, input.cardAddress, holders);
    const found = new Map();
    await Promise.all(holders.flatMap((holder) => candidates.map(async (tid) => {
 ...
 */
async function resolveMergedCardOwnership(input) {
    const card = new ethers_1.ethers.Contract(input.cardAddress, OWNERSHIP_ABI, input.provider);
    let eoa = '';
    let aa = '';
    try {
        eoa = ethers_1.ethers.getAddress(String(input.eoa || '').trim());
    }
    catch {
        eoa = '';
    }
    try {
        aa = input.aaAddress ? ethers_1.ethers.getAddress(String(input.aaAddress).trim()) : '';
    }
    catch {
        aa = '';
    }
 ...
 */
async function holderHasValidMembershipNft(input) {
    const ownership = await resolveMergedCardOwnership(input);
    return ownership.nfts.some((n) => (0, membershipFeeMetadata_1.isMembershipNftTokenId)(n.tokenId) && !n.isExpired);
}
