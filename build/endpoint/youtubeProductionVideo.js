...
exports.normalizeYoutubeProductionVideoUrl = normalizeYoutubeProductionVideoUrl;
exports.youtubeEmbedUrlFromVideoId = youtubeEmbedUrlFromVideoId;
exports.decodeYoutubeUrlPathSegment = decodeYoutubeUrlPathSegment;
exports.youtubeChannelUsernameFromAuthorUrl = youtubeChannelUsernameFromAuthorUrl;
exports.extractYoutubeShortDescriptionFromWatchHtml = extractYoutubeShortDescriptionFromWatchHtml;
exports.validateYoutubeProductionVideoUrl = validateYoutubeProductionVideoUrl;
...
function normalizeYoutubeProductionVideoUrl(raw) {
    const id = parseYoutubeVideoId(raw);
    if (!id)
        return null;
    return `https://www.youtube.com/watch?v=${id}`;
}
function youtubeEmbedUrlFromVideoId(videoId) {
    return `https://www.youtube.com/embed/${videoId}?rel=0`;
}
/** oEmbed often returns percent-encoded handles in `author_url` (e.g. `/@%E7%9D%A1…`). */
function decodeYoutubeUrlPathSegment(segment) {
...
function youtubeChannelUsernameFromAuthorUrl(authorUrl, authorName) {
    ...
    if (url) {
        try {
            ...
            if (handleMatch?.[1])
                return decodeYoutubeUrlPathSegment(handleMatch[1]);
            const userMatch = parsed.pathname.match(/^\/user\/([^/?#]+)/);
            if (userMatch?.[1])
                return decodeYoutubeUrlPathSegment(userMatch[1]);
            const cMatch = parsed.pathname.match(/^\/c\/([^/?#]+)/);
            if (cMatch?.[1])
                return decodeYoutubeUrlPathSegment(cMatch[1]);
        }
        catch {
            // fall through
        }
...
function extractYoutubeShortDescriptionFromWatchHtml(html) {
...
async function fetchYoutubeVideoDescription(videoId, signal) {
    ...
    try {
        const resp = await fetch(watchUrl, {
            ...
            headers: {
                ...
                'User-Agent': 'Mozilla/5.0 (compatible; BeamioYoutubeValidator/1.0; +https://beamio.app)',
            },
            signal: merged,
        });
...
async function validateYoutubeProductionVideoUrl(rawUrl) {
